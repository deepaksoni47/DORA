...
// Convert a hex color like "#ca9a5b" into RGB integers 0-1
...
export default function SplashCursor({
  ...
  TRANSPARENT = true,
  colorHex = "#ca9a5b" // DORA accent color
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Instead of completely random colors, we use the specified colorHex and slight blending
      const base = hexToRGB(colorHex);
      // Small random variations
      base.r = base.r * 0.5 + Math.random() * 0.15;
      base.g = base.g * 0.5 + Math.random() * 0.15;
      base.b = base.b * 0.5 + Math.random() * 0.15;
      return base;
    }
    ...
    return () => {
      // Cleanup events to avoid multiple fluid instances if component remounts
      document.body.removeEventListener('mousemove', handleFirstMouseMove);
      document.body.removeEventListener('touchstart', handleFirstTouchStart);
    };
  }, [
  ...
  return (
    <div className="fixed inset-0 z-0 pointer-events-none w-full h-full opacity-60">
      <canvas ref={canvasRef} id="fluid" className="w-screen h-screen block pointer-events-none"></canvas>
    </div>
  );
}
